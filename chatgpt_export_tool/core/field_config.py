...
class FieldSelector:
    ...
    # Field categories for categorization
    CATEGORIES: Dict[str, List[str]] = {
        ...
        "metadata": []  # Catch-all for unknown fields
    }
    
    def __init__(self, mode: str, fields: List[str] = None, groups: List[str] = None):
            ...
            for group_name in self.groups:
                if group_name in self.GROUPS:
                    ...
                    if group_name in self.CATEGORIES:
                        selected.update(self.CATEGORIES[group_name])
    ...
    def filter_node(self, node: Dict[str, Any]) -> Dict[str, Any]:
        """Filter mapping node fields.
        
        Args:
            node: Node dictionary from mapping.
            
        Returns:
            Filtered node dictionary.
        """
        all_fields = set(node.keys())
        selected = self.get_selected_fields(all_fields)
        return {k: v for k, v in node.items() if k in selected}
    
    def filter_message(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        all_fields = set(msg.keys())
        selected = self.get_selected_fields(all_fields)
        return {k: v for k, v in msg.items() if k in selected}
